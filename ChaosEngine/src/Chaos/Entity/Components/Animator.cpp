...
namespace Chaos
{
    ...
    void Animator::Play()
    {
        ...
        if (GetEntity()->HasComponent<Render>())
        {
            GetEntity()->GetComponent<Render>()->SetActive(false);  
        }
        m_play = true;
        m_time = 0;
    }
    ...
    {   
        ...
        if (GetEntity()->HasComponent<Render>())
        {
            GetEntity()->GetComponent<Render>()->SetActive(true);  
        }
        m_play = false;
        m_time = 0;
    }
    ...
    void Animator::SetAnimation(Animation animation)
    {
        ...
        m_spriteSheetBounds = Vec2((float)animation.SpriteSheet->GetWidth() / animation.FrameSize.X,
                                    (float )animation.SpriteSheet->GetHeight() / animation.FrameSize.Y); // getting max coordinates for x and y given the frame size and the width
        m_length = (float)animation.TotalFrames / (float)animation.FrameRate;   //calculating length in seconds
