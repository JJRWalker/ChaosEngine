...
namespace Chaos
{
	ImGuiFileExplorer::ImGuiFileExplorer(char** filePath)
	{
		m_filePath = filePath;
	...
	void ImGuiFileExplorer::OnImGuiUpdate()
	{
		...
		if (ImGui::Button("^"))
		{
			strcpy(m_tempPath, std::filesystem::proximate(*m_filePath, m_rootPath).parent_path().string().c_str());
			LOGCORE_TRACE("{0}",std::filesystem::proximate(*m_filePath, m_rootPath).parent_path().string().c_str());
		}
		ImGui::BeginChild("ScrollingRegion", ImVec2(0, -30), false, ImGuiWindowFlags_HorizontalScrollbar);
		if (std::filesystem::exists(std::filesystem::proximate(*m_filePath, m_rootPath)) )
		{
			int i = 0;
			static int selected = 0;
			if (std::filesystem::proximate(*m_filePath, m_rootPath).extension() == "")
			{
				for (const auto& entry : std::filesystem::directory_iterator(std::filesystem::proximate(*m_filePath, m_rootPath)))
				{
					i++;
			...
			{
				for (const auto& entry : std::filesystem::directory_iterator(std::filesystem::proximate(*m_filePath, m_rootPath).parent_path()))
				{
					i++;
					if (ImGui::Selectable(entry.path().string().c_str(), selected == i))
					{
