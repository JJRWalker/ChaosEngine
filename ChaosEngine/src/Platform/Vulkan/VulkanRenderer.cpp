...
{	VulkanRenderer::VulkanRenderer(Window* window) : pWindow(window)
	...
	void VulkanRenderer::DrawObjects(VkCommandBuffer cmd, VulkanRenderObject* first, size_t count)
	{
		...
		{
			VulkanRenderObject& obj = first[i];
			sobjectSSBO[i].ModelMatrix = obj.Transform;
			sobjectSSBO[i].Optional[0][0] = 23.4f;
		}
		...
		for (int i = 0; i < count; ++i)
		{
			...
			vkCmdPushConstants(cmd, obj.Material->PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(MeshPushConstants), &constants);
...
// the size of the pool is very oversize, but it's copied from imgui demo itself.
	...
	void VulkanRenderer::InitPipelines()
	{
		...
		VulkanMaterial defaultMat(pipeline, pipelineLayout, "defaultMesh");
		UploadMaterial(defaultMat);


		VkPipelineLayoutCreateInfo texturedPipelineLayoutInfo = pipelineLayoutInfo;
		...
		if (!LoadShaderModule("../ChaosEngine/shaders/spv/textured_lit.frag.spv", &texturedFragShader))
		{
			LOGCORE_WARN("Failed to load triangle frag shader module");
		}
		...
		pipelineBuilder.ShaderStages.push_back(VkInit::PipelineShaderStageCreateInfo(VK_SHADER_STAGE_VERTEX_BIT, vertShader));
		pipelineBuilder.ShaderStages.push_back(VkInit::PipelineShaderStageCreateInfo(VK_SHADER_STAGE_FRAGMENT_BIT, texturedFragShader));
		...
		VulkanMaterial texMat(texPipeline, texturedPipelineLayout, "texturedMesh");
		UploadMaterial(texMat);

		vkDestroyShaderModule(m_device, fragShader, nullptr);
		vkDestroyShaderModule(m_device, vertShader, nullptr);
		vkDestroyShaderModule(m_device, texturedFragShader, nullptr);
	...
	void VulkanRenderer::InitScene()
	{
		...
		VulkanMaterial* texturedMat = GetMaterial("texturedMesh");
		...
		imageBufferInfo.imageView = Textures["lost_empire"].ImageView;
	...
	void VulkanRenderer::LoadMeshes()
	{
		...
		VulkanMesh monkeyMesh{};
		monkeyMesh.LoadFromOBJ("../Game/Assets/monkey_smooth.obj");

		VulkanMesh lostEmpire{};
		lostEmpire.LoadFromOBJ("../Game/Assets/lost_empire.obj");
	...
	{
		VulkanTexture tex = VulkanTexture(this, "");
		UploadTexture(tex);
	}
